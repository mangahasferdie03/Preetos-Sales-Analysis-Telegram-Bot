...
{undelivered_formatted}"""
    ...
    async def send_scheduled_sales_report(self, application):
        ...
        try:
            ...
            if not chat_id:
                logger.warning("REPORT_CHAT_ID not set - skipping scheduled report")
                return

            logger.info(f"Sending scheduled sales report to chat {chat_id}")
            ...
            # Get the sales data
            result = self.analyze_sales_for_dates(date_str, date_str)

            if result is None:
                await application.bot.send_message(
                    chat_id=chat_id,
                    text="⚠️ Could not generate scheduled sales report - no data available."
                )
                return

            # Send the report
            message = f"📊 **Automated Sales Report** - {today.strftime('%B %d, %Y')}\n\n{result}"
            await application.bot.send_message(
                chat_id=chat_id,
                text=message,
                parse_mode='Markdown'
            ...
            logger.info("Scheduled sales report sent successfully")
    ...
    def setup_scheduler(self, application):
        ...
        timezone_str = os.getenv('TIMEZONE', 'Asia/Manila')
        timezone = pytz.timezone(timezone_str)

        # Create scheduler
        scheduler = AsyncIOScheduler(timezone=timezone)

        # Schedule reports at 3 PM
        scheduler.add_job(
    ...
    def run(self):
        ...
        try:
            ...
            print("📅 Scheduled reports enabled at 3 PM and 11 PM")
